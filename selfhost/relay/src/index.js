...
function setupPage(origin) {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#03070A"><title>Flud Companion Relay</title><style>:root{color-scheme:dark;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(90% 35% at 50% -5%,rgba(23,184,187,.16),transparent 68%),#020507;color:#f5f8fa;padding:24px 14px}main{max-width:620px;margin:auto}.brand{text-align:center;margin:12px 0 22px}.brand img{width:88px;height:88px;border-radius:24px}.brand h1{margin:10px 0 0;font-size:30px}.brand h1 span{color:#17b8bb;font-weight:450}.brand small{color:#7e8b96;letter-spacing:.18em}.card{border:1px solid rgba(120,181,201,.18);border-radius:24px;padding:22px;background:rgba(8,14,18,.88)}.ready{color:#68dc93;font-weight:700}.muted{color:#8d99a7;line-height:1.55}.url{display:block;overflow-wrap:anywhere;background:#03070b;border:1px solid rgba(127,168,184,.22);border-radius:15px;padding:14px;margin:16px 0}.actions{display:grid;grid-template-columns:1.2fr .8fr;gap:10px}.button,button{border:0;border-radius:15px;min-height:50px;padding:0 16px;font-weight:750;text-decoration:none;display:grid;place-items:center;cursor:pointer}.primary{background:linear-gradient(135deg,#34e0df,#12acc4);color:#001518}.secondary{background:#071017;border:1px solid rgba(126,173,190,.25);color:#dbe3e8}.steps{margin:22px 0 0;padding:0;list-style:none;counter-reset:s;color:#cdd6db}.steps li{counter-increment:s;display:grid;grid-template-columns:30px minmax(0,1fr);gap:10px;align-items:start;padding:10px 0;border-bottom:1px solid rgba(127,168,184,.10)}.steps li:before{content:counter(s);width:26px;height:26px;border-radius:50%;display:grid;place-items:center;background:rgba(23,184,187,.08);border:1px solid rgba(23,184,187,.22);color:#17b8bb;font-size:12px;font-weight:700}.stepText{min-width:0;line-height:1.55;overflow-wrap:anywhere}.stepText b{font-weight:750;color:#eef6f7}@media(max-width:460px){.actions{grid-template-columns:1fr}.card{padding:18px}.steps li{grid-template-columns:28px minmax(0,1fr);gap:11px}.stepText{font-size:14px}}</style></head><body><main><header class="brand"><img src="/icon.svg" alt="Flud Companion"><h1>Flud <span>Companion</span></h1><small>alexlab.media</small></header><section class="card"><div class="ready">● Remote relay online · ${VERSION}</div><h2>Your relay is ready</h2><p class="muted">This relay belongs to your Cloudflare account. There is no shared Flud Companion cloud account.</p><code id="relay" class="url">${safe}</code><div class="actions"><button id="copy" class="primary">Copy relay URL</button><a class="button secondary" href="/app">Open Remote PWA</a></div><ol class="steps"><li><div class="stepText">Open Flud Companion Bridge on the Android device.</div></li><li><div class="stepText">Choose <b>Quick setup → LAN + Remote</b>.</div></li><li><div class="stepText">Paste this relay URL once.</div></li><li><div class="stepText">Scan the <b>Remote QR</b> with your phone.</div></li></ol></section></main><script>copy.onclick=async()=>{try{await navigator.clipboard.writeText(location.origin);copy.textContent='Copied ✓'}catch(e){const r=document.createRange();r.selectNodeContents(relay);const s=getSelection();s.removeAllRanges();s.addRange(r)}}</script></body></html>`;
}
